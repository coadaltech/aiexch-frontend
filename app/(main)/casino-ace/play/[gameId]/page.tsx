 ...
 *
 * Fills the parent <main> scroll container exactly (h-full) so there is no
 * nested page scroll — only the game inside the iframe scrolls. A slim
 * toolbar sits on top; the iframe takes all remaining height.
 */
export default function CasinoAcePlayPage({
  ...
  const [state, setState] = useState<
    | { kind: "loading" }
    | { kind: "ready"; url: string; sessionToken: string; gameName: string }
    ...
    (async () => {
      try {
        ...
        setState({
          kind: "ready",
          url: res.data.url,
          sessionToken: res.data.playerSessionToken,
