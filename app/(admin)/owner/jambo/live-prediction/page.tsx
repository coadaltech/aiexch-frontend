...
import {
  useOwnerJamboShifts,
  useJamboLivePrediction,
  useJamboLivePredictionWhitelabels,
  useJamboAgentSale,
  useDeclareJamboResult,
  useJamboDeclaredHistory,
  type LivePredictionWhitelabelRow,
} from "@/hooks/useOwner";
import { useAuth } from "@/contexts/AuthContext";
import { usePermissions } from "@/contexts/PermissionContext";
import { JamboJantriGridModal } from "./jantri-modal";
...
export default function OwnerJamboLivePredictionPage() {
  const { user: currentUser } = useAuth();
  const { has } = usePermissions();
  // Reuse the matka declare permission (same key gates the jambo declare API).
  const isOwner = has("live_prediction.declare");
  void currentUser;

  const { data: shifts = [], isLoading: shiftsLoading } = useOwnerJamboShifts();
  ...
  const { data: livePrediction, isLoading: gridLoading } =
    useJamboLivePrediction(shiftId || null);
  const numbers = livePrediction?.numbers ?? [];

  const [amtDesc, setAmtDesc] = useState(true);
  // Only list numbers that carry a sale (1..1000 is too long to show in full).
  const sortedNumbers = useMemo(() => {
    const active = numbers.filter(
      (n) =>
        Number(n.sale) !== 0 ||
        Number(n.profit) !== 0 ||
        (n.declared_count ?? 0) > 0
    );
  ...
  const { data: whitelabels = [], isLoading: wlLoading } =
    useJamboLivePredictionWhitelabels(shiftId || null, selectedNum);
  const { data: agentSales = [], isLoading: agentSaleLoading } =
    useJamboAgentSale(shiftId || null, selectedNum);
  ...
  // Jantri is keyed by the party (user): no party selected → all users'
  // consolidated grid; a selected party → just that user's grid (mirrors matka).
  ...
  const handleDeclare = () => {
    ...
    if (!shiftId || isNaN(n) || n < 0 || n > JAMBO_MAX_NUMBER) return;
    if (jantri.msLeft > 0) {
  ...
  return (
    <div className="fixed top-14 left-0 lg:left-64 right-0 bottom-0 flex flex-col overflow-hidden bg-gray-200">
      ...
      >
        <span className="font-bold">Jambo Live Prediction</span>
      ...
      <div className="flex items-stretch flex-1 gap-0.5 p-1 min-h-0 overflow-x-auto overflow-y-hidden">
          ...
          <div className="overflow-y-auto flex-1">
            {gridLoading ? (
              ...
              <div className="py-10 text-center text-gray-400 text-[11px]">
                No bets placed on this shift yet.
              </div>
            ) : (
                ...
                >
                  ...
                  <div
                    className={`flex-1 py-1.5 text-center font-semibold ${
                      Number(row.profit) >= 0 ? "text-[#22c55e]" : "text-red-500"
                    }`}
                  >
                    {Number(row.profit) < 0 ? "-" : ""}
                    {fmt(row.profit)}
        ...
        <div className="flex flex-col bg-white border border-gray-300 rounded overflow-hidden text-sm flex-1 min-w-[320px]">
            ...
            {selectedNum == null ? (
              ...
              sortedWhitelabels.map((wl, i) => {
                ...
                return (
                  <div
                    key={wl.user_id}
                    onClick={() => setSelectedParty(wl)}
                    className={`flex items-center border-b border-gray-100 cursor-pointer hover:bg-gray-50 ${
                      selectedParty?.user_id === wl.user_id ? "bg-teal-50" : ""
                    }`}
                  >
                      ...
                      {wl.streak > 0 && wl.streak_type != null && (
                        <span
                          ...
                          className={`inline-flex items-center justify-center min-w-5 h-5 px-1 rounded font-bold text-[10px] text-white ${
                            wl.streak_type === 1 ? "bg-[#22c55e]" : "bg-red-500"
                          }`}
                        >
          ...
          >
            <span>
              {selectedParty ? `Party: ${selectedParty.name}` : "All users"} |
              Number: {selectedNum ?? "—"}
            </span>
            <button
              ...
              title={
                selectedParty
                  ? `Jantri for ${selectedParty.name}`
                  : "Jantri for all users"
              }
      ...
      <JamboJantriGridModal
        open={jantriOpen}
        onClose={() => setJantriOpen(false)}
        shiftId={shiftId || null}
        partyId={selectedParty?.user_id ?? "all"}
        partyName={selectedParty ? selectedParty.name : "All"}
      />
